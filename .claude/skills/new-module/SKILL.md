...
| 4   | `<module>.service.ts`      | Facade public API — service TRÙNG TÊN module, đặt ở gốc. Business logic ở đây. Lỗi = `throw new DomainException(<Module>Errors.X, msg, HttpStatus.<4xx>)` (enum, không số trần). Config qua `ConfigService<CoreApiEnv, true>` + `{ infer: true }`, cấm `process.env` trực tiếp. Thao tác không được lặp → transaction DB + prefix idempotency key theo domain (builder trong `<module>.constants.ts`). List → query `limit + 1` rồi `buildCursorPage()`. |
| 4b  | Thành phần con (nếu có)    | Xếp theo VAI TRÒ đúng bảng quyết định docs/05 § 5.3: sub-service nghiệp vụ → `services/`; job nền định kỳ → `jobs/`; port/strategy đổi impl qua env → `ports/`; helper gọi API bên thứ 3 → `clients/`; webhook → `webhooks/`; store riêng → `redis/`. Gốc module chỉ giữ facade. Folder chưa cần thì KHÔNG tạo sẵn.                                                                                                                                      |
| 4c  | `<module>.constants.ts`    | Mọi hằng có ngữ nghĩa vượt ra ngoài 1 file (hằng bên thứ 3, tham số sản phẩm/bảo mật, Kafka topic, tên constraint, prefix/builder idempotency key) khai ở đây NGAY TỪ ĐẦU — không đợi nơi dùng thứ 2 (docs/05 § 5.1). Kèm comment lý do từng cái. Chỉ hằng vận hành nội bộ của đúng 1 class (batch size, tên job, Lua script) mới khai tại file đó.                                                                                                      |
