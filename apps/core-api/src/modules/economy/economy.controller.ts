...
export class EconomyController {
  ...
  verifyIap(@CurrentUser() user: AuthenticatedUser, @Body() dto: VerifyIapDto) {
    return this.economyService.creditFromIap(user.userId, dto.provider, dto.payload, dto.productId);
  }
  ...
  listTransactions(@CurrentUser() user: AuthenticatedUser, @Query() query: CursorPageQueryDto) {
    return this.economyService.listTransactions(user.userId, query.limit, query.cursor);
  }
}
