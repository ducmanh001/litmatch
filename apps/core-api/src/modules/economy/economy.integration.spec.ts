...
d('Economy integration (Postgres thật)', () => {
  ...
  it('IAP credit: gửi receipt 2 lần chỉ credit 1 lần (idempotency theo provider txn id)', async () => {
    const first = await economy.creditFromIap(
      userA,
      IapProvider.Google,
      { devTransactionId: 'gpa-0001' },
      'com.litmatch.diamond.100',
    );
    expect(first.replayed).toBe(false);
    const second = await economy.creditFromIap(
      userA,
      IapProvider.Google,
      { devTransactionId: 'gpa-0001' },
      'com.litmatch.diamond.100',
  ...
  it('2 request song song KHÁC key, ví chỉ đủ cho 1 → đúng 1 thành công, số dư không âm (docs/10 § Economy)', async () => {
    ...
    const poor = await userRepo.save(
      userRepo.create({
        nickname: 'test-poor',
        avatarId: 'default-01',
        isGuest: false,
    ...
    await economy.creditFromIap(
      poor.id,
      IapProvider.Google,
      { devTransactionId: `gpa-poor-${poor.id}` },
      'com.litmatch.diamond.550',
    );
  ...
  it('reversal: hoàn tiền bằng bút toán đảo, reverse lần 2 → 409, ledger gốc còn nguyên', async () => {
    const purchase = await economy.purchaseVip(
      userA,
      'vip-30d',
      'to-be-reversed',
    );
    ...
    await expect(
      ledger.reverse(purchase.transactionId, 'reversal-key-2', 'reverse lần 2'),
    ).rejects.toMatchObject({
      code: EconomyErrors.TRANSACTION_ALREADY_REVERSED,
    });
    const original = await ds
      .getRepository(LedgerTransaction)
      .findOneByOrFail({ id: purchase.transactionId });
  ...
  it('trigger DB chặn UPDATE/DELETE ledger_entries — append-only không phụ thuộc kỷ luật code', async () => {
    await expect(
      ds.query(`UPDATE ledger_entries SET amount = 999999 WHERE true`),
    ).rejects.toThrow(/append-only/);
    await expect(
      ds.query(`DELETE FROM ledger_entries WHERE true`),
    ).rejects.toThrow(/append-only/);
  });
  ...
  it('đối soát: phát hiện snapshot bị sửa lệch, rebuildWallet khôi phục từ ledger', async () => {
    ...
    await ds.query(
      `UPDATE wallets SET balance = balance + 7777 WHERE user_id = $1`,
      [userA],
    );
  ...
  it('lịch sử giao dịch: cursor pagination + diamondDelta đúng dấu', async () => {
    ...
    const page2 = await economy.listTransactions(
      userA,
      2,
      page1.meta.nextCursor as string,
    );
    expect(page2.data.map((t) => t.id)).not.toEqual(
  ...
  it('refund sau khi đã tiêu: balance âm đúng, tiêu tiếp bị chặn, nạp bù trừ nợ đúng (docs/services/economy-service.md § 5)', async () => {
    ...
    const credit = await economy.creditFromIap(
      user.id,
      IapProvider.Google,
      { devTransactionId: 'refund-case-1' },
      'com.litmatch.diamond.1200',
    );
    ...
    const unknown = await refundService.refundIapPurchase(
      IapProvider.Google,
      'khong-ton-tai',
      'test',
    );
    ...
    const receipt = await ds
      .getRepository(IapReceipt)
      .findOneByOrFail({ transactionId: credit.transactionId });
    const outcome = await refundService.refundIapPurchase(
      receipt.provider,
      receipt.providerTransactionId,
      'test hoàn tiền store',
    );
    ...
    const replay = await refundService.refundIapPurchase(
      receipt.provider,
      receipt.providerTransactionId,
      'retry webhook',
    );
    ...
    await expect(
      economy.purchaseVip(user.id, 'vip-30d', 'refund-case-vip-2'),
    ).rejects.toMatchObject({
      code: EconomyErrors.WALLET_INSUFFICIENT_BALANCE,
    });
    ...
    await economy.creditFromIap(
      user.id,
      IapProvider.Google,
      { devTransactionId: 'refund-case-2' },
      'com.litmatch.diamond.550',
    );
    ...
    for (let step = 0; step < 60; step++) {
      ...
      try {
        if (dice < 0.45) {
          ...
          const r = await economy.creditFromIap(
            user.id,
            IapProvider.Google,
            { devTransactionId: `prop-credit-${step}` },
            product,
          );
          reversibleTransactionIds.push(r.transactionId);
        } else if (dice < 0.8) {
          const r = await economy.purchaseVip(
            user.id,
            'vip-30d',
            `prop-vip-${step}`,
          );
          ...
          const candidates = reversibleTransactionIds.filter(
            (id) => !alreadyReversed.has(id),
          );
