...
describe('MatchingService (unit — mock repo/redis/economy)', () => {
  let ticketRepo: jest.Mocked<Pick<Repository<MatchTicket>, 'save' | 'create' | 'findOneBy' | 'findOneByOrFail' | 'increment'>>;
  let redis: { eval: jest.Mock; zadd: jest.Mock; zrem: jest.Mock; sadd: jest.Mock; decr: jest.Mock };
  ...
  beforeEach(() => {
    ticketRepo = {
      ...
      increment: jest.fn(async () => ({ affected: 1, raw: [], generatedMaps: [] })),
    } as never;
    redis = {
    ...
    economy = {
      spendDiamond: jest.fn(async () => ({ transactionId: 'txn-1', replayed: false })),
    ...
    dataSource = { transaction: jest.fn(async (cb: (m: EntityManager) => Promise<unknown>) => cb(manager as never)) };
  ...
  describe('joinQueue', () => {
    it('derive region + ageBand từ profile server-side, KHÔNG nhận từ client (docs/10 § 10.0.B)', async () => {
      const ticket = await service.joinQueue(me, { matchType: MatchType.Voice }, 'k1');
      ...
      expect(redis.sadd).toHaveBeenCalledWith('matching:shards:active', 'matching:queue:voice:VN:5');
    });

    it('profile thiếu region/birthDate → shard GLOBAL + ageBand -1 (không chặn, không đoán)', async () => {
      userService.getByIdOrThrow.mockResolvedValue({ id: me.userId, status: UserStatus.Active, region: null, birthDate: null } as never);
      const ticket = await service.joinQueue(me, { matchType: MatchType.Soul }, 'k1');
    ...
    it('user bị ban → MATCHING_USER_BANNED 403, không tạo ticket', async () => {
      userService.getByIdOrThrow.mockResolvedValue({ id: me.userId, status: UserStatus.Banned, region: 'VN', birthDate: null } as never);
      await expect(service.joinQueue(me, { matchType: MatchType.Voice }, 'k1')).rejects.toMatchObject({
    ...
    it('retry cùng Idempotency-Key → trả lại ticket cũ + re-enqueue NX, không tạo ticket đôi', async () => {
      ...
      ticketRepo.save.mockRejectedValueOnce({ code: '23505', message: 'uq_match_tickets_idempotency_key' });
      ticketRepo.findOneBy.mockResolvedValueOnce(existing);

      const ticket = await service.joinQueue(me, { matchType: MatchType.Voice }, 'k1');
      expect(ticket).toBe(existing);
      expect(redis.zadd).toHaveBeenCalledWith(expect.any(String), 'NX', expect.any(String), existing.id);
    });

    it('cùng key nhưng nội dung khác (matchType đổi) → 409 IDEMPOTENCY_CONFLICT (docs/05 § 5.10)', async () => {
      ticketRepo.save.mockRejectedValueOnce({ code: '23505', message: 'uq_match_tickets_idempotency_key' });
      ticketRepo.findOneBy.mockResolvedValueOnce(makeTicket({ matchType: MatchType.Soul }));
      await expect(service.joinQueue(me, { matchType: MatchType.Voice }, 'k1')).rejects.toMatchObject({
    ...
    it('đã có ticket active (partial unique index bắn 23505) → 409 ALREADY_QUEUED', async () => {
      ticketRepo.save.mockRejectedValueOnce({ code: '23505', message: 'uq_match_tickets_active_user' });
      ticketRepo.findOneBy.mockResolvedValueOnce(null); // không phải replay
      await expect(service.joinQueue(me, { matchType: MatchType.Voice }, 'k-khac')).rejects.toMatchObject({
  ...
  describe('ownership / IDOR (docs/10 § 10.1.D)', () => {
    it('getTicket của user khác → 403 FORBIDDEN', async () => {
      ticketRepo.findOneBy.mockResolvedValueOnce(makeTicket({ userId: 'user-khac' }));
      await expect(service.getTicket(me, 'ticket-1')).rejects.toMatchObject({ code: MatchingErrors.TICKET_FORBIDDEN });
    });

    it('getTicket không tồn tại → 404', async () => {
      ticketRepo.findOneBy.mockResolvedValueOnce(null);
      await expect(service.getTicket(me, 'ticket-x')).rejects.toMatchObject({ code: MatchingErrors.TICKET_NOT_FOUND });
    });

    it('cancelTicket của user khác → 403, không đổi trạng thái', async () => {
      manager.findOne.mockResolvedValueOnce(makeTicket({ userId: 'user-khac' }));
      await expect(service.cancelTicket(me, 'ticket-1')).rejects.toMatchObject({ code: MatchingErrors.TICKET_FORBIDDEN });
      expect(manager.save).not.toHaveBeenCalled();
    });
  });

  describe('state machine (spec § 1) — không tin client gửi trạng thái đích', () => {
    it.each([MatchTicketStatus.Matched, MatchTicketStatus.Confirmed, MatchTicketStatus.Expired, MatchTicketStatus.Cancelled])(
      'cancel khi ticket đang %s → 409 INVALID_TRANSITION',
      async (status) => {
    ...
    it('confirm khi ticket còn queued (chưa được ghép) → 409 INVALID_TRANSITION', async () => {
      ticketRepo.findOneBy.mockResolvedValueOnce(makeTicket({ status: MatchTicketStatus.Queued }));
    ...
    it('cancel hợp lệ: queued→cancelled + ZREM khỏi shard', async () => {
      ...
      expect(redis.zrem).toHaveBeenCalledWith('matching:queue:voice:VN:5', 'ticket-1');
    });
  });
  ...
  describe('speedup (spec § 4)', () => {
    beforeEach(() => {
      ...
      ticketRepo.findOneByOrFail.mockResolvedValue(makeTicket({ priorityBoostMs: 300_000 }));
    });

    it('rate-limit vượt giới hạn → 409 RATE_LIMITED và KHÔNG gọi spendDiamond (chặn trước khi trừ tiền)', async () => {
      redis.eval.mockResolvedValueOnce(-1);
      await expect(service.speedup(me, 'ticket-1', 'sk1')).rejects.toMatchObject({
    ...
    it('happy path: rate-limit TRƯỚC spendDiamond, rồi boost DB + ZADD XX score tuyệt đối', async () => {
      ...
      const expectedScore = new Date('2026-07-12T00:00:00Z').getTime() - 300_000;
      expect(redis.zadd).toHaveBeenCalledWith('matching:queue:voice:VN:5', 'XX', String(expectedScore), 'ticket-1');
      expect(result.replayed).toBe(false);
    });

    it('replay cùng key → KHÔNG cộng boost lần 2, hoàn lại slot rate-limit (docs/10 § 10.0.D)', async () => {
      economy.spendDiamond.mockResolvedValueOnce({ transactionId: 'txn-1', replayed: true });
      const result = await service.speedup(me, 'ticket-1', 'sk1');
      expect(result.replayed).toBe(true);
      expect(ticketRepo.increment).not.toHaveBeenCalled();
      expect(redis.decr).toHaveBeenCalledWith(`matching:speedup:count:${me.userId}`);
      // vẫn sửa lại score Redis từ tổng boost DB (retry-hoàn-tất an toàn, spec § 4)
      expect(redis.zadd).toHaveBeenCalledWith(expect.any(String), 'XX', expect.any(String), 'ticket-1');
    });

    it('spendDiamond fail (không đủ diamond) → hoàn slot rate-limit, không boost', async () => {
      economy.spendDiamond.mockRejectedValueOnce(new DomainException('ECONOMY_WALLET_INSUFFICIENT_BALANCE', 'x', 422));
      await expect(service.speedup(me, 'ticket-1', 'sk1')).rejects.toMatchObject({
    ...
    it('speedup ticket không còn queued → 409, không đụng rate-limit/tiền', async () => {
      ticketRepo.findOneBy.mockResolvedValueOnce(makeTicket({ status: MatchTicketStatus.Matched }));
      await expect(service.speedup(me, 'ticket-1', 'sk1')).rejects.toMatchObject({
        code: MatchingErrors.TICKET_INVALID_TRANSITION,
      });
