...
export const SOUL_SESSION_REFETCH_INTERVAL_MS = 10_000;
export const SOUL_MESSAGES_REFETCH_INTERVAL_MS = 5_000;
...
export function useSoulSession(sessionId: string) {
  return useQuery({
    ...
    refetchInterval: (query) =>
      isOpenPhase(query.state.data?.phase)
...
export function useSoulMessages(sessionId: string) {
  return useInfiniteQuery({
    ...
    refetchInterval: SOUL_MESSAGES_REFETCH_INTERVAL_MS,
  });
}
