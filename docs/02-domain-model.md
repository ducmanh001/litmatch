...
| **Friend graph** — `friend`                                  | `Friendship`, `ProfileFollow`, `ProfileChatContact`, `Conversation`, `ConversationMemberState`, `Message`, `ConversationStreak`                                                | Friendship tạo conversation bền vững; profile follow độc lập với first-contact; chat profile mở trực tiếp hoặc sau gift theo N lượt/ngày UTC; block cắt send/read. |
...
User ─┬─ Wallet ─ Transaction ─ LedgerEntry ─ LedgerAccount
      ...
      ├─ Friendship / ProfileFollow / ProfileChatContact ─ Conversation ─ Message ─ ConversationStreak
