...
| **Matching** — `matching`                                    | `MatchTicket`, `MatchSession`, `MatchInvite`, `GuestMatchQuota`; `MatchQueue` là Redis index                                                                                   | Một user chỉ có một active queue/ticket; pair/accept re-check consent/safety; guest quota khóa theo ngày UTC và các key chống farm đã hash.                  |
...
| **Calling** — `calling`                                      | `CallSession`, `VoiceMatchReaction`                                                                                                                                            | Media room là LiveKit state; server giữ quyền/timer/billing; mutual-like phải atomically tạo quan hệ bền vững.                                               |
...
- LiveKit room/participant và Socket.IO connection là transport/runtime state; quyền, billing,
  membership và lifecycle vẫn thuộc Core API.
