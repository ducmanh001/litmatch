...
3. Google OAuth là đường đăng nhập production; phone OTP, video upload, IAP và
   external push được tắt fail-closed do chưa có vendor credential/adapter miễn phí an toàn.
...
| --------------------------------------------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| Production không khởi động dev provider             | Operator chọn `dev` trong production                             | `auth/ports/sms-provider.ts:26`, `economy/ports/iap-verifier.ts:73`, `notification/ports/push-provider.ts:42`, video storage/transcode bootstrap guards | ✅      |
| Phone OTP tắt trước khi sinh/lưu/gửi OTP            | Client gọi thẳng request/verify endpoint                         | `auth/auth.service.ts:57-64`, guard `:166-173`; unit và production smoke 403                                                                            | ✅      |
...
| UI capability khớp backend                          | Operator build frontend với phone OTP bật                        | release build ép hai frontend flag `false` tại `scripts/release/release.mjs:94-106`; Compose ép backend `false` tại `deploy/production/compose.yml:95`  | ✅      |
...
| Core API thật với `NODE_ENV=production`, PostgreSQL/Redis, provider disabled                                                             | `/health/ready` 200; OTP request 403 `AUTH_PHONE_OTP_DISABLED`                           |
...
VM public IP, Google OAuth client và secret trong `deploy/production/.env`. Bốn capability cần
vendor trả phí/credential (phone OTP, video upload, IAP, external push) được tắt minh bạch
và fail-closed; các chức năng còn lại, in-app notification, Google login, realtime/calling,
admin, monitoring và release lifecycle giữ nguyên trong profile này.
