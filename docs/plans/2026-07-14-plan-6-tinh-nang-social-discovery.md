...
- Report mở rộng `targetType ('user'|'video')` trong safety — video report tái dùng trust penalty pipeline.
...
- Upload qua presigned URL — body video không bao giờ chạm NestJS. 2 port: `VideoStoragePort`,
  `VideoTranscodePort` + Dev impl chặn cứng production (pattern DevSmsProvider). Vendor = ADR sau.
...
- Preset → auto-approve; **free text → PendingReview**. Gap thật: repo **chưa có** hệ moderation
  text — dựng `ModerationPort` (pattern PushPort/SmsProvider), MVP = blocklist qua config + admin
...
| **W5** | **Video ngắn V1** hướng Momo (lifecycle + Dev ports + like/comment + view counting + report/admin + profile/party-room integration)                                                                     | ~2–3 tuần          |
| Sau    | **Feed P2/P3/P5 (chỉ mở lại khi số liệu đòi)**, mood free-text (sau khi chốt moderation), video V2/V3 (share icebreaker, vendor ADR, auto-scan), streak freeze qua economy, `feed_affinity` incremental | theo quyết định mở |
