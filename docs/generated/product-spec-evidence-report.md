...
| ------------------------- | ----------------------------------------------------------------------------- | -------------------------------------------------- | --------------------- |
| auth-session              | Guest, OTP, social login, identity upgrade and rotating browser sessions      | core-api/auth                                      | automated-test-source |
| runtime-capabilities      | Server-derived runtime availability contract for frontend providers           | core-api/app with admin/web consumers              | automated-test-source |
| economy-ledger            | Wallet, IAP catalog, VIP and double-entry ledger                              | core-api/economy                                   | automated-test-source |
| matching                  | Queue ticket, pairing, guest quota and paid speed-up                          | core-api/matching                                  | automated-test-source |
...
### Guest, OTP, social login, identity upgrade and rotating browser sessions
...
- Status: `implemented`
- Owner: core-api/app with admin/web consumers
- Contracts: `openapi/core-api.json`, `docs/12-frontend-architecture.md`, `.env.example`
- Implementation evidence: `apps/core-api/src/app/capabilities.controller.ts:10` — contains `@Controller('capabilities')`; `apps/core-api/src/app/capabilities.service.ts:29` — contains `getCapabilities(): CapabilitiesDto`
- Verification evidence (automated-test-source): `apps/core-api/src/app/capabilities.service.spec.ts:35` — contains `derives configured providers`; `apps/web/src/shared/capabilities/api.spec.tsx:9` — contains `capabilities`
...
- Source evidence: `apps/core-api/src/arch/module-layout.spec.ts:76` — contains `chỉ để HTTP facade chính ở root`; `apps/core-api/src/modules/matching/controllers/invite.controller.spec.ts:1` — contains `InviteController`
...
- Evidence: `docs/07-roadmap.md:99` — contains `số liệu traffic production thật`
