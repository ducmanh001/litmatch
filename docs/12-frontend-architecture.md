...
libs/
  ...
  browser-auth/        adapter SDK định danh browser dùng chung, không chứa UI/business rule
openapi/
...
`libs/browser-auth` là adapter browser thuần cho SDK định danh bên thứ ba được cả web và admin
dùng chung. Lib chỉ tải SDK, lấy credential và trả lỗi kỹ thuật; OAuth client id thuộc env của
từng app, REST exchange vẫn đi qua `api-client`, còn UI/điều hướng vẫn thuộc app. Lib không sở
hữu session, role, business rule hay component dùng chung.
...
5. Locale UI dùng `vi` hoặc `en`; preference thuộc app, còn mọi REST phải gửi
   `Accept-Language` qua `@litmatch/api-client` để core-api localize system message. Không dịch
   error theo message string: `error.code` vẫn là contract cho behavior.
6. Server state dùng TanStack Query; client state đi theo ladder ở docs 13.
7. Không tạo `libs/ui` hay abstraction để dành. Lib mới cần cập nhật architecture trước.
8. Next server code tuân boundary 12.5; secret không bao giờ dùng prefix public.
9. Form nhập dữ liệu validate bằng Zod; backend vẫn validate và quyết định nghiệp vụ.
10. Trước khi báo xong chạy `pnpm agent:verify frontend`; command PASS mới là bằng chứng máy.
11. Phát hiện docs/spec sai thì sửa nguồn canonical trong cùng thay đổi.
