...
- [x] **Discovery — browse theo tiêu chí (W1, plan tại [docs/plans/2026-07-14-plan-6-tinh-nang-social-discovery.md](./plans/2026-07-14-plan-6-tinh-nang-social-discovery.md)):** module mới `discovery` — filter gender/tuổi/khu vực kiểu dating app (khác bộ lọc giới tính lúc ghép cặp ở mục ngay trên — mục đó áp cho hàng đợi matching, mục này là màn duyệt/tìm chủ động), loại trừ block+report 2 chiều qua `SafetyService.getHiddenUserIds` (report ẩn **vĩnh viễn**, không cooldown như matching), card trả `ageBucket` (composition riêng, KHÔNG sửa `PublicProfileDto` dùng chung ở Soul Match/Friend) — [services/discovery-service.md](./services/discovery-service.md). CTA sau khi thấy user ở bản này: **chỉ xem profile**; re-chốt 2026-07-14 (lăng kính dating-first): sẽ bổ sung CTA **"mời Voice/Soul Match"** tái dùng matching engine — hạng mục W4 cùng đợt nearby (mục backlog dưới), không sửa lại bản đã code. **`review-module verify` PASS + đã commit (d48a161)** — phát hiện lúc verify: filter khu vực ban đầu định dùng `User.region` (field hạ tầng LiveKit/Matching, không có đường ghi thật cấp thành phố) nên đã bỏ khỏi W1, dành cho Nearby.
