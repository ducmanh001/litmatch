...
| FE REST chỉ qua generated api-client              | docs/12 § 12.3                        | guard fetch/axios + OpenAPI drift check                                         | `openapi:check`, `openapi-contract.test.mjs`, api-client tests | Frontend/API           |
...
| Contract/skill adapter không trôi                 | AGENTS + docs/08 § 8.2.1              | `adapter-parity`: coverage + exact target trên working tree hoặc Git index      | `adapter-parity.test.mjs`                                      | Agent/platform         |
| Startup context không inject roadmap/backlog      | docs/08 § 8.2.1                       | Session-start chỉ in stable routing, workspace risk và gate                     | `session-start.test.mjs`                                       | Agent/platform         |
| Tên identifier nói đúng domain và contract        | docs/17                               | Review tay; formatter chỉ hỗ trợ presentation, không chứng minh ngữ nghĩa       | PR diff + checklist § 17.8                                     | Author/reviewer        |
...
| Secret/PII không vào source/log/image             | docs/05 § 5.7–5.8, Security policy    | logger redact, Gitleaks/CodeQL/Trivy CI workflow disabled; audit cần review tay | n/a                                                            | Security/platform      |
